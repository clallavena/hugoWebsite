...
Project of IT Bachelor's Degree.

HAL is a Hyper Monitoring project meant for retrieve some data in an IT park and send them into a server managed by the SysAdmin, in goal to agregate the data and handle them.
To do so, HAL use a plugin system, which are loaded automaticaly at the start of the client.

Severals languages are available for writing the plugins:
* C/C++/C#/Go (.dll / .so)
* Go Script (.go)
* Python (.py)
* Ruby (.rb)
...
### README of the project (french only)
